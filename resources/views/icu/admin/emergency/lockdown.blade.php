...
@section('content')
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div class="min-w-0">
            <h1 class="text-2xl font-semibold tracking-tight">Manual System Lockdown</h1>
            <div class="text-sm text-slate-500">Lock down access during incident response (DB-backed).</div>
        </div>
    ...
    <div class="mt-6 grid grid-cols-1 xl:grid-cols-3 gap-4">
        <div class="bg-white border border-slate-200/80 rounded-2xl p-5">
            ...
            <div class="mt-3">
                @if ((bool) ($s?->lockdown_enabled ?? false))
                    <x-status-badge variant="critical">Lockdown Enabled</x-status-badge>
                @else
                    <x-status-badge variant="stable">No Lockdown</x-status-badge>
                @endif
            </div>
            <div class="mt-3 text-sm text-slate-500">Scope: {{ $s?->lockdown_scope ?? 'admin' }}</div>
            <div class="mt-2 text-sm text-slate-500">Reason: {{ $s?->lockdown_reason ?? '—' }}</div>
        </div>

        <div class="xl:col-span-2 bg-white border border-slate-200/80 rounded-2xl p-5">
            <div class="text-sm font-semibold">Update Lockdown</div>
            <div class="mt-1 text-sm text-slate-500">Stores state in DB and records an emergency event + audit log.</div>

            <form method="POST" action="{{ route('admin.emergency.lockdown.update') }}" class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                @csrf

                <div class="md:col-span-2 flex items-center gap-2">
                    <input type="checkbox" name="lockdown_enabled" value="1" class="rounded border-slate-300" @checked((bool) old('lockdown_enabled', $s?->lockdown_enabled ?? false)) />
                    <div class="text-sm">Enable lockdown</div>
                </div>

                <div>
                    <div class="text-xs text-slate-500">Scope</div>
                    @php $sc = old('lockdown_scope', $s?->lockdown_scope ?? 'admin'); @endphp
                    <select name="lockdown_scope" class="mt-2 w-full h-11 rounded-xl border border-slate-200/80 bg-white px-3 text-sm">
                        <option value="admin" @selected($sc === 'admin')>admin</option>
                        <option value="all" @selected($sc === 'all')>all</option>
                        <option value="api_only" @selected($sc === 'api_only')>api_only</option>
                    </select>
                </div>

                <div>
                    <div class="text-xs text-slate-500">Reason</div>
                    <input name="lockdown_reason" value="{{ old('lockdown_reason', $s?->lockdown_reason) }}" class="mt-2 w-full h-11 rounded-xl border border-slate-200/80 bg-white px-3 text-sm" placeholder="e.g. suspected compromise" />
                </div>

                <div class="md:col-span-2">
                    <button type="submit" class="w-full h-11 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-sm font-medium">Apply Lockdown</button>
                </div>
            </form>
        </div>
    </div>
